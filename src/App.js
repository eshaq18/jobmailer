...
function LoginScreen({ onLogin }) {
  ...
  const handle = () => { if (pass !== PASS) { setErr('كلمة المرور غير صحيحة'); return; } onLogin(); };
  return (
    <div className="login-wrap">
      <div className="login-card">
        ...
        <div className="field-group">
          ...
          <input type="password" placeholder="أدخل كلمة المرور" value={pass}
            onChange={e => { setPass(e.target.value); setErr(''); }}
            onKeyDown={e => e.key === 'Enter' && handle()} className="inp" />
...
export default function App() {
  ...
  const handleExcel = e => {
    ...
    reader.onload = ev => {
      ...
      setSheets(result); setSelectedCities([]);
    };
  ...
  const fill = (tpl, row) => tpl
    ...
    .replace(/{{ContactName}}/g, row.ContactName || row.Name || row['الاسم'] || 'مسؤول التوظيف')
  ...
  const fetchStats = async () => {
    const updated = await Promise.all(
      sendLog.map(async l => {
        ...
        try {
          const r = await fetch('/api/stats', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messageId: l.msgId }) });
            ...
            {Object.keys(sheets).length > 0 && (
              <div className="card">
                ...
                <div className="city-grid">
                  <button className={`city-btn ${selectedCities.length === 0 ? 'active' : ''}`} onClick={() => setSelectedCities([])}>
                    الكل <span className="city-count">{Object.values(sheets).flat().length}</span>
                  </button>
                  {Object.keys(sheets).map(city => (
                    <button key={city} className={`city-btn ${selectedCities.includes(city) ? 'active' : ''}`} onClick={() => toggleCity(city)}>
              ...
              <div className="card">
                ...
                <div className="qty-grid">
                  ...
                  {[100,200,300,400,500,600,700,800,900].map(n => (
                    <button key={n} className={`qty-btn ${qtyMode === 'preset' && qtyPreset === n ? 'active' : ''}`}
                      onClick={() => { setQtyMode('preset'); setQtyPreset(n); }}>{n}</button>
                  ))}
                  <button className={`qty-btn ${qtyMode === 'custom' ? 'active' : ''}`} onClick={() => setQtyMode('custom')}>تحديد يدوي</button>
                </div>
                {qtyMode === 'custom' && (
        ...
        {activeTab === 2 && (
          <div className="tab-content">
            <div className="stats-grid">
              ...
              <div className="stat-card"><span className="stat-label">فشل</span><span className="stat-val red">{failed}</span></div>
              <div className="stat-card"><span className="stat-label">الوقت المنقضي</span><span className="stat-val">{fmt(elapsed)}</span></div>
            ...
            {sendLog.length > 0 && (
              <div className="card">
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                  ...
                  <span style={{fontSize:12,color:'var(--muted)'}}>✓ أُرسل &nbsp; 📬 وصل &nbsp; 👁 فُتح</span>
                </div>
          ...
          <div className="tab-content">
            <div className="card">
              ...
              <div className="report-grid">
                ...
                <div className="report-item"><span className="report-icon">❌</span><span className="report-num red">{failed}</span><span className="report-lbl">فشل</span></div>
                <div className="report-item"><span className="report-icon">⏱</span><span className="report-num">{fmt(elapsed)}</span><span className="report-lbl">وقت الجلسة</span></div>
